...
export function PublishSkillPage() {
  ...
  const [packageIdAvailable, setPackageIdAvailable] = useState<boolean | null>(
    null,
  );
  ...
  const validateStep = (s: number): string | null => {
    if (s === 0) {
      if (
        !formData.packageId.match(/^[a-z0-9][a-z0-9-]{1,98}[a-z0-9]$/) &&
        formData.packageId.length < 3
      )
        return "Package ID must be at least 3 characters, lowercase letters, numbers and hyphens only.";
      if (packageIdAvailable === false)
        return "This Package ID is already taken.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pkg = await fetchApi<{ id: string; packageId: string }>(
        "/v1/packages",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  return (
    <div className="app-container">
      ...
      >
        ...
        >
          Your knowledge rules are AES-256 encrypted at rest. They never leave
          the platform in plaintext.
        </p>
          ...
          {step === 2 && (
            <div>
              ...
              >
                ...
                Your rules are sent over TLS and immediately encrypted with
                AES-256-GCM at the server. The plaintext never touches disk —
                only the ciphertext is stored. Rules are decrypted only in
                volatile memory during sandbox execution.
              </div>
              ...
              >
                To use CLI instead: <code>yigyaps publish</code> — the CLI reads
                from <code>yigyaps.yaml</code> in your project directory
              </div>
            </div>
          )}
