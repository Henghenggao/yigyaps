...
async function main() {
  ...
  try {
    ...
    console.log(
      `[Phase 2] Simulating EVR (Encrypted Virtual Room) Invocation...`,
    );
    ...
    const conclusion = await SecureBuffer.withSecureContext(
      ...
      async (dekBuffer) => {
        console.log(
          `  -> Inside Software Enclave: Decrypting rules directly to memory...`,
        );
        ...
        console.log(
          `  -> Rules decrypted successfully (${plaintextRules.length} characters)`,
        );

        console.log(
          `  -> Running Agent Logic (simulated format without leaking rules)...`,
        );
    ...
    console.log(
      `  🛡️ Raw rules securely processed in memory and instantly wiped!\n`,
    );
