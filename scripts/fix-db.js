...
async function run() {
  ...
  try {
    ...
    await client.query(
      "ALTER TABLE yy_skill_package_installations ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'installing';",
    );
    console.log("Ensured 'status' column in yy_skill_package_installations.");

    // 3. Mark all current packages as active
    await client.query(
      "UPDATE yy_skill_packages SET status = 'active' WHERE status IS NULL;",
    );
  } catch (err) {
