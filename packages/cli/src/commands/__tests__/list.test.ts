...
describe("listCommand", () => {
    ...
    it("should list installed skills in a table", async () => {
        ...
        const mockClient = {
            getInstallations: vi.fn().mockResolvedValue({ installations: mockInstallations })
        };
    ...
    it("should output JSON when --json flag is set", async () => {
        const mockInstallations = [{ id: "inst1", packageId: "pkg", agentId: "a1", status: "active", installedAt: 0 }];
        const mockClient = {
            getInstallations: vi.fn().mockResolvedValue({ installations: mockInstallations })
        };
        vi.mocked(registry.createRegistryClient).mockReturnValue(
